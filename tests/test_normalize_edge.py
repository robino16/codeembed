...
def test_relation_uppercased():
    edge = GraphAnalysisResultEdge(source="A", relation="calls", target="B")
    assert _normalize_edge(edge).relation == "CALLS"


def test_relation_spaces_replaced_with_underscores():
    edge = GraphAnalysisResultEdge(source="A", relation="is supplier of", target="B")
...
def test_source_whitespace_stripped():
    edge = GraphAnalysisResultEdge(source="  AuthService.login  ", relation="CALLS", target="B")
    assert _normalize_edge(edge).source == "AuthService.login"


def test_target_whitespace_stripped():
    edge = GraphAnalysisResultEdge(source="A", relation="CALLS", target="  jwt_decode  ")
    assert _normalize_edge(edge).target == "jwt_decode"


def test_already_normalized_unchanged():
    edge = GraphAnalysisResultEdge(source="AuthService.login", relation="CALLS", target="JwtService.sign")
